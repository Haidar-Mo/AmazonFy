...
class WalletAddressesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($old_address)
            return $this->showMessage('address.validation.network_name.unique', status: 400, success: false);
        return DB::transaction(function () use ($request) {
            ...
            return $this->showResponse($wallet->load('walletAddress'), 'address.store_success');
    ...
    public function update(Request $request, WalletAddress $walletAddress)
    {
        ...
        return DB::transaction(function () use ($request, $wallet, $address) {
            $request->validate([
                ...
                'target' => ['string']
            ], ['name.unique' => 'address.validation.network_name.unique']);
            $address->update($request->all());
            $address->save();
            return $this->showResponse($wallet->load('walletAddress'), 'address.update_success');
        });
    }
